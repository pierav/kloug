...
class spike(pluginTemplate):
    ...
    def runTests(self, testList, cgf_file=None):
        ...
        for file in testList:
            ...
            compile_cmd = self.compile_cmd.format(testentry['isa'].lower(), self.xlen) + ' ' + test + ' -o ' + elf
            compile_cmd += ' -D' + " -D".join(testentry['macros'])
            compile_cmd += '; ' + self.objdump_cmd.format(elf, self.xlen, 'ref.disass')

            simcmd = self.ref_exe + ' --isa={0} +signature={1} +signature-granularity=4 {2}'.format(self.isa, sig_file, elf)
            
            # concatenate all commands that need to be executed within a make-target.
            execute = '@cd {0}; {1}; {2};'.format(testentry['work_dir'], compile_cmd, simcmd)
